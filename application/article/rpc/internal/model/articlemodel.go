...
type (
	...
	ArticleModel interface {
		articleModel
		ArticlesByUserId(ctx context.Context, userId string, likeNum int64, pubTime, sortField string, limit int) ([]*Article, error)
		UpdateArticleStatus(ctx context.Context, id string, status int) error
	}
...
func (m *customArticleModel) ArticlesByUserId(ctx context.Context, userId string, likeNum int64, pubTime, sortField string, limit int) ([]*Article, error) {
	var (
		err      error
		sql      string
		anyField any
...
func (m *customArticleModel) UpdateArticleStatus(ctx context.Context, id string, status int) error {
	beyondArticleArticleIdKey := fmt.Sprintf("%s%v", cacheGozeroArticleArticleIdPrefix, id)
