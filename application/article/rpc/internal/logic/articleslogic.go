...
const (
	prefixArticles = "biz#articles#%s#%d"
...
func (l *ArticlesLogic) Articles(in *pb.ArticlesRequest) (*pb.ArticlesResponse, error) {
	l.ctx, _ = context.WithTimeout(context.Background(), time.Second*200)
	...
	if len(articleIds) > 0 {
		...
		v, err, _ := l.svcCtx.SingleFlightGroup.Do(fmt.Sprintf("ArticlesByUserId:%s:%d", in.UserId, in.SortType), func() (interface{}, error) {
...
func (l *ArticlesLogic) articleByIds(ctx context.Context, articleIds []string) ([]*model.Article, error) {
	articles, err := mr.MapReduce[string, *model.Article, []*model.Article](func(source chan<- string) {
	...
	}, func(id string, writer mr.Writer[*model.Article], cancel func(error)) {
...
func (l *ArticlesLogic) cacheArticles(ctx context.Context, uid string, cursor, ps int64, sortType int32) ([]string, error) {
...
func (l *ArticlesLogic) addCacheArticles(ctx context.Context, articles []*model.Article, userId string, sortType int32) error {
	if len(articles) == 0 {
		return nil
	}
	...
	for _, article := range articles {
		...
		_, err := l.svcCtx.BizRedis.ZaddCtx(ctx, key, score, article.Id)
		if err != nil {
			return err
		}
	}
